...
- **Monaco Editor Worker Imports**: Resolved Monaco Editor worker import path issues for Vite/Rolldown production build compatibility.

### Changed

- **Natural Sorting for Branches & Tags**: Implemented natural alphanumeric sorting (`natural_cmp`) for tags and branches in Git repository management so tags like `v0.1.0`, `v0.2.0`, `v0.10.0` sort logically.
