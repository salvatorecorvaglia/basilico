...
export function FileViewerModal() {
  ...
  const { fileViewerOpen, fileViewerPath, fileViewerOid, closeFileViewer, addNotification } = useUIStore();
  ...
  useEffect(() => {
    ...
    getFileContentAtRevision(activeTabId, fileViewerPath, fileViewerOid)
      ...
      .catch((err) => {
        ...
        addNotification({ type: 'error', message: `Failed to load file content: ${err}` });
        setContent('');
      })
      .finally(() => {
        setLoading(false);
      });
  }, [fileViewerOpen, fileViewerPath, fileViewerOid, activeTabId, addNotification]);
  ...
  const handleDownload = () => {
    ...
    addNotification({ type: 'success', message: `Saved ${filename} to downloads` });
  };

  return (
    <div className="file-viewer-overlay animate-fade-in" onClick={closeFileViewer}>
      <div className="file-viewer-content" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="file-viewer-header">
          <div className="file-viewer-title-group">
            <span className="file-viewer-name">{fileViewerPath.split('/').pop()}</span>
            <span className="file-viewer-path text-mono truncate" title={fileViewerPath}>
              {fileViewerPath} @ {fileViewerOid.slice(0, 7)}
            </span>
          </div>

          <div className="file-viewer-actions">
            <button className="viewer-action-btn" onClick={handleCopy} title="Copy content" disabled={loading}>
              {copied ? <Check size={14} className="text-success" /> : <Copy size={14} />}
              <span>{copied ? 'Copied' : 'Copy'}</span>
            </button>
            <button className="viewer-action-btn" onClick={handleDownload} title="Save to disk" disabled={loading}>
              <Download size={14} />
              <span>Save</span>
            </button>
