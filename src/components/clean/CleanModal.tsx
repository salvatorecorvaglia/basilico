...
import { AlertTriangle, Trash2, Folder, File, X, ChevronRight, CheckCircle } from 'lucide-react';
    ...
    <div className="clean-modal-overlay animate-fade-in" onClick={handleClose}>
      <div className="clean-modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="clean-modal-header">
          ...
          <button className="clean-modal-close-btn" onClick={handleClose} disabled={isSubmitting}>
            <X size={16} />
          </button>
        </div>
        ...
        <div className="clean-wizard-steps">
          <span className={`wizard-step-node ${step === 'setup' ? 'active' : ''}`}>1. Configure</span>
          <ChevronRight size={12} className="wizard-step-sep" />
          <span className={`wizard-step-node ${step === 'preview' ? 'active' : ''}`}>2. Preview</span>
          <ChevronRight size={12} className="wizard-step-sep" />
          <span className={`wizard-step-node ${step === 'result' ? 'active' : ''}`}>3. Completed</span>
        </div>
          ...
          {step === 'setup' && (
            <div className="clean-step-setup animate-fade-in">
              <p className="clean-desc">
                Clean operations remove untracked files and directories from your working tree.
              </p>
              ...
              <div className="clean-options-group">
                <label className="clean-option-checkbox">
                  ...
                  <div className="option-label-details">
                    <span className="option-title">Clean Untracked Directories (-d)</span>
                    <span className="option-desc">Remove entire directories that are not tracked by Git.</span>
                  </div>
                </label>
                ...
                <label className="clean-option-checkbox option-warning-label">
                  ...
                  />
                  <div className="option-label-details">
                    <span className="option-title">Include Ignored Files (-x)</span>
                    <span className="option-desc">
                      Remove files and directories matched by your `.gitignore` rules (e.g. build targets, node_modules).
                    </span>
                  </div>
                </label>
              </div>
              ...
              {includeIgnored && (
                <div className="clean-warning-callout">
                  ...
                  <span>
                    Warning: Including ignored files can wipe out configurations, secrets, build artifacts, or dependencies. Proceed with caution.
                  </span>
                </div>
              )}
            </div>
              ...
              {paths.length === 0 ? (
                ...
                <>
                  <div className="preview-heading">
                    ...
                    <span>The following {paths.length} item(s) will be permanently deleted:</span>
                  </div>

                  <div className="preview-paths-list text-mono">
                    {paths.map((p, idx) => (
                      <div key={idx} className="preview-path-item">
                        {p.endsWith('/') ? <Folder size={12} className="path-icon-folder" /> : <File size={12} className="path-icon-file" />}
                        <span className="path-text truncate" title={p}>{p}</span>
                      </div>
                    ))}
                  </div>
                </>
        ...
        <div className="clean-modal-footer">
          {step === 'setup' && (
            <>
              <button className="clean-btn clean-btn-secondary" onClick={handleClose}>
                Cancel
              </button>
              <button 
              ...
              >
                {isSubmitting ? <span className="spinner-small" /> : 'Scan Repository (Dry Run)'}
              </button>
            </>
          )}
            ...
            <>
              ...
              {paths.length > 0 ? (
                <button 
                  ...
                  disabled={isSubmitting}
                >
                  {isSubmitting ? <span className="spinner-small" /> : 'Clean Repository (Delete Files)'}
                </button>
              ) : (
                <button className="clean-btn clean-btn-secondary" onClick={handleClose}>
                  Close
