...
export function BlameView() {
  ...
  const [currentCommitOid, setCurrentCommitOid] = useState<string | null>(selectedCommitOid);

  useEffect(() => {
    if (selectedFilePath) {
      loadFileBlame(selectedFilePath, currentCommitOid);
    }
  }, [selectedFilePath, currentCommitOid]);

  if (!selectedFilePath) {
    return (
      <div className="blame-view-empty">
        <p>No file selected for blame. Please select a file from staging or history.</p>
        <button className="blame-btn" onClick={() => setActiveView('graph')}>Back to History</button>
      </div>
    );
  }
  ...
  return (
    <div className="blame-view animate-fade-in">
      <div className="blame-header">
        <button className="blame-back-btn" onClick={() => setActiveView('graph')} title="Back to Graph">
          <ArrowLeft size={14} />
          <span>Back</span>
        </button>
        <div className="blame-file-info truncate">
          ...
          {currentCommitOid && (
            <span className="blame-revision text-mono text-secondary">
              &nbsp;@ {currentCommitOid.includes('^') ? currentCommitOid : currentCommitOid.slice(0, 8)}
            </span>
          )}
        </div>
        {currentCommitOid && (
        ...
        {isLoading ? (
          ...
          <div className="blame-lines text-mono">
            {blameLines.map((line, idx) => {
              ...
              const colorGroup = isNaN(oidInt) ? 0 : (oidInt % 6);
              ...
              return (
                <div key={idx} className="blame-line-row">
                  ...
                  <div className={`blame-gutter color-group-${colorGroup}`}>
                    ...
                    <span className="blame-gutter-author truncate" title={`${line.authorName} <${line.authorEmail}>`}>
                      {line.authorName}
                    </span>
                    {line.commitOid && (
                      <button 
                  ...
                  {/* Line Number */}
                  <div className="blame-line-number text-tertiary">{line.lineNo}</div>

                  {/* Line Content */}
                  <pre className="blame-line-content">{line.lineContent || ' '}</pre>
                </div>
              );
            })}
          </div>
