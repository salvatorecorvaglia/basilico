...
import { X, Check, ChevronLeft, ChevronRight, CheckSquare, AlertTriangle } from 'lucide-react';
...
export function MergeEditor() {
  ...
  useEffect(() => {
    ...
    const fetchStages = async () => {
      try {
        ...
        addNotification({ type: 'error', message: `Failed to load conflict files: ${err}` });
    ...
    fetchStages();
  }, [activeConflictedPath, loadConflictStages, setActiveView, addNotification]);
  ...
  useEffect(() => {
    if (conflictBlocks.length > 0 && activeBlockIndex >= conflictBlocks.length) {
  ...
  const handleResolveBlock = (choice: 'ours' | 'theirs' | 'both') => {
    ...
    if (choice === 'ours') {
      ...
      replacement = block.ours + (block.ours && block.theirs ? '\n' : '') + block.theirs;
    }
      ...
      <div className="merge-header">
        <div className="merge-header-title">
          ...
          <div className="merge-header-info">
            ...
            <span className="text-mono text-secondary truncate">{activeConflictedPath}</span>
          </div>
        </div>
        ...
        <div className="merge-header-actions">
          {conflictBlocks.length > 0 ? (
            <span className="conflict-badge warning">
              {conflictBlocks.length} conflict{conflictBlocks.length > 1 ? 's' : ''} remaining
            </span>
          ) : (
          ...
          <button className="merge-btn merge-btn-outline" onClick={() => setActiveView('staging')}>
            <X size={14} /> Cancel
          </button>
          ...
          {conflictBlocks.length > 0 && (
            <div className="conflict-control-bar">
              ...
              <div className="conflict-resolutions">
                <button className="resolve-btn resolve-ours" onClick={() => handleResolveBlock('ours')}>
                  Accept Ours
                </button>
                <button className="resolve-btn resolve-theirs" onClick={() => handleResolveBlock('theirs')}>
                  Accept Theirs
                </button>
                <button className="resolve-btn resolve-both" onClick={() => handleResolveBlock('both')}>
                  Accept Both
                </button>
              </div>
            </div>
