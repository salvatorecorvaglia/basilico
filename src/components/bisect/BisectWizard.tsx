...
export function BisectWizard() {
  ...
  const handleStart = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!goodCommit.trim()) {
      addNotification({ type: 'warning', message: 'Good commit is required to start bisecting' });
      return;
    }
  ...
  const handleMark = async (status: 'good' | 'bad' | 'skip') => {
    try {
      ...
      addNotification({ type: 'info', message: `Marked commit as ${status.toUpperCase()}` });
    } catch (err) {
      addNotification({ type: 'error', message: `Failed to mark commit: ${err}` });
    }
  };
  ...
  const currentCommitDetails = commits.find(c => c.oid === bisectState?.currentOid);

  // If not bisecting, show setup form
  if (!bisectState || !bisectState.isBisecting) {
    return (
      <div className="bisect-wizard setup animate-fade-in">
        <div className="bisect-header">
          ...
          <p className="subtitle text-tertiary">
            Binary search your repository history to find the commit that introduced a bug.
          </p>
        </div>
        ...
        <form onSubmit={handleStart} className="bisect-setup-form">
          <div className="form-group">
            ...
            />
            <span className="help-text">Usually the latest commit containing the bug.</span>
          </div>
          ...
          <div className="form-group">
            ...
            <input 
              ...
              required
            />
            <span className="help-text">A past commit where you verify the code worked correctly.</span>
          </div>

          <button type="submit" className="bisect-submit-btn" disabled={isStarting}>
            <Play size={14} />
    ...
    <div className="bisect-wizard active-session animate-fade-in">
      <div className="bisect-header">
        <div className="bisect-header-left">
          ...
          <span className={`badge-bisect-state ${isFinished ? 'finished' : 'testing'}`}>
            {isFinished ? 'Finished' : 'Testing'}
          </span>
        </div>
        <button className="bisect-abort-btn" onClick={handleReset} title="Abort Bisect">
          <RotateCcw size={12} />
            ...
            <div className="bisect-status-card">
              ...
              <div className="bisect-steps-info">
                {bisectState.stepsRemaining !== null ? (
                  <>
                    <span>Roughly <b>{bisectState.stepsRemaining}</b> steps remaining</span>
                    <span className="commits-left text-tertiary">({bisectState.message.split('after this').shift()?.trim()})</span>
                  </>
                ) : (
            ...
            <div className="bisect-commit-card">
              ...
              {bisectState.currentOid ? (
                <div className="commit-test-details">
                  <div className="commit-sha text-mono text-success">{bisectState.currentOid}</div>
                  {currentCommitDetails ? (
                    <>
                      <div className="commit-message truncate">{currentCommitDetails.message}</div>
