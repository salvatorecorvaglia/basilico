...
export function parseAutolinks(
  text: string,
  ...
  const forge = remoteUrl ? parseRemoteUrl(remoteUrl) : null;
  const defaultIssueUrlPattern = forge
    ? `${forge.webBaseUrl}/issues/<id>`
    : null;

  const urlTemplate = autolinkUrl || defaultIssueUrlPattern;
  const regexPattern = autolinkPattern
    ? new RegExp(autolinkPattern, "g")
    : /#(\d+)/g;
  ...
  while (match !== null) {
    ...
    const issueId = match[1] || matchedText.replace(/^#/, "");
