use git2::{ApplyLocation, ApplyOptions, Repository, build::CheckoutBuilder};
use std::path::Path;
...
pub async fn unstage_files(path: String, files: Vec<String>) -> Result<(), String> {
    ...
    match head {
        Ok(head_ref) => {
            ...
            repo.reset_default(Some(commit.as_object()), &files).map_err(|e| e.to_string())?;
        }
        Err(_) => {
...
pub async fn apply_patch(
    ...
    repo.apply(&diff, apply_loc, Some(&mut apply_opts)).map_err(|e| e.to_string())?;
    Ok(())
}
...
pub async fn discard_changes(path: String, files: Vec<String>) -> Result<(), String> {
    ...
    repo.checkout_index(None, Some(&mut opts)).map_err(|e| e.to_string())?;
    Ok(())
}
